#!/usr/bin/env node

/**
 * Round 4 — Minimum viable clips to make all 6 problems usable.
 *
 * Priority: fix problems that are missing hook OR treatment (not yet renderable).
 * 6 clips × 8 credits = 48 credits. Estimated balance ~304 → leaves ~256 (above floor 250).
 *
 * After this run all 6 problems (blocked-drain, burst-pipe, leaking-tap,
 * greasy-rangehood, dirty-bathroom, end-of-lease) will have ≥1 hook + ≥1 treatment.
 *
 * Usage:
 *   node src/video/kling-batch-round4.js
 *   node src/video/kling-batch-round4.js --dry-run
 */

import '../utils/load-env.js';
import { createHmac } from 'crypto';
import { mkdirSync, createWriteStream, existsSync } from 'fs';
import { parseArgs } from 'util';
import path from 'path';

const KLING_ACCESS_KEY = process.env.KLING_ACCESS_KEY;
const KLING_SECRET_KEY = process.env.KLING_SECRET_KEY;
const KLING_API_BASE   = 'https://api.klingai.com/v1';
const CLIPS_ROOT       = new URL('../../clips', import.meta.url).pathname;

const CREDITS_PER_CLIP = 8;
const CREDIT_FLOOR     = 250;
let estimatedCredits   = 304;  // after round 3

const { values: args } = parseArgs({
  options: { 'dry-run': { type: 'boolean', default: false } },
  strict: false,
});

function klingJwt() {
  const now = Math.floor(Date.now() / 1000);
  const header  = Buffer.from(JSON.stringify({ alg: 'HS256', typ: 'JWT' })).toString('base64url');
  const payload = Buffer.from(JSON.stringify({ iss: KLING_ACCESS_KEY, exp: now + 1800, nbf: now - 5 })).toString('base64url');
  const sig = createHmac('sha256', KLING_SECRET_KEY).update(`${header}.${payload}`).digest('base64url');
  return `${header}.${payload}.${sig}`;
}

async function submit(prompt) {
  const res = await fetch(`${KLING_API_BASE}/videos/text2video`, {
    method: 'POST',
    headers: { 'Authorization': `Bearer ${klingJwt()}`, 'Content-Type': 'application/json' },
    body: JSON.stringify({ model_name: 'kling-v3', prompt, duration: '5', aspect_ratio: '9:16', mode: 'pro', cfg_scale: 0.7 }),
  });
  if (!res.ok) throw new Error(`submit failed ${res.status}: ${(await res.text()).substring(0, 200)}`);
  const data = await res.json();
  const taskId = data?.data?.task_id;
  if (!taskId) throw new Error(`missing task_id: ${JSON.stringify(data)}`);
  return taskId;
}

async function pollOne(taskId) {
  const res = await fetch(`${KLING_API_BASE}/videos/text2video/${taskId}`, {
    headers: { 'Authorization': `Bearer ${klingJwt()}` },
  });
  if (!res.ok) throw new Error(`poll failed ${res.status}`);
  const data = await res.json();
  const task = data?.data;
  return { status: task?.task_status, url: task?.task_result?.videos?.[0]?.url };
}

async function pollBatch(tasks, maxWaitMs = 360000) {
  const start = Date.now();
  const remaining = new Set(tasks.map((_, i) => i));
  const results = new Array(tasks.length).fill(null);
  process.stdout.write('  Polling');
  while (remaining.size > 0 && Date.now() - start < maxWaitMs) {
    await sleep(5000);
    process.stdout.write('.');
    for (const i of [...remaining]) {
      const { status, url } = await pollOne(tasks[i].taskId);
      if (status === 'succeed') { process.stdout.write(` ✓${tasks[i].name}`); results[i] = url; remaining.delete(i); }
      else if (status === 'failed') { process.stdout.write(` ✗${tasks[i].name}`); remaining.delete(i); }
    }
  }
  console.log('');
  return results;
}

async function download(url, destPath) {
  const res = await fetch(url);
  if (!res.ok) throw new Error(`download failed ${res.status}`);
  const dest = createWriteStream(destPath);
  await new Promise((resolve, reject) => {
    res.body.pipeTo(new WritableStream({
      write(chunk) { dest.write(chunk); },
      close() { dest.end(); resolve(); },
      abort(err) { dest.destroy(); reject(err); },
    }));
  });
  return `${(dest.bytesWritten / 1024 / 1024).toFixed(1)}MB`;
}

function sleep(ms) { return new Promise(r => setTimeout(r, ms)); }

// ─── Clip definitions ─────────────────────────────────────────────────────────

const S = 'no logos or text on screen, handheld camera, realistic, Australian';

const ALL_CLIPS = [
  // Fix leaking-tap: needs hook (has treatment-a)
  { name: 'leaking-tap-hook-a', file: 'leaking-tap-hook-a.mp4', dir: 'plumbing/leaking-tap',
    prompt: `kitchen tap dripping constantly, water drops falling into sink, homeowner frustrated watching water waste, ${S} home` },

  // Fix dirty-bathroom: needs treatment (has hook-a)
  { name: 'dirty-bathroom-treatment-a', file: 'dirty-bathroom-treatment-a.mp4', dir: 'house-cleaning/dirty-bathroom',
    prompt: `professional cleaner applies grout cleaner and scrubs bathroom tiles until gleaming white, methodical and thorough, ${S} bathroom` },

  // Fix end-of-lease: needs hook + treatment (has nothing)
  { name: 'end-of-lease-hook-a', file: 'end-of-lease-hook-a.mp4', dir: 'house-cleaning/end-of-lease',
    prompt: `tenant walks through empty rental apartment looking stressed, inspecting dirty walls and carpet stains before moving-out inspection, ${S}` },
  { name: 'end-of-lease-treatment-a', file: 'end-of-lease-treatment-a.mp4', dir: 'house-cleaning/end-of-lease',
    prompt: `professional cleaners transform empty rental apartment, scrubbing oven, steam cleaning carpet, every surface spotless, ${S}` },

  // Add variety: blocked-drain treatment-b
  { name: 'blocked-drain-treatment-b', file: 'blocked-drain-treatment-b.mp4', dir: 'plumbing/blocked-drain',
    prompt: `plumber uses electric drain snake to clear blocked kitchen sink, methodical technique, ${S} home` },

  // Add variety: burst-pipe hook-b
  { name: 'burst-pipe-hook-b', file: 'burst-pipe-hook-b.mp4', dir: 'plumbing/burst-pipe',
    prompt: `homeowner discovers water pouring from burst pipe in ceiling, grabs bucket in panic, ${S} home` },
];

function chunkOf3(arr) {
  const chunks = [];
  for (let i = 0; i < arr.length; i += 3) chunks.push(arr.slice(i, i + 3));
  return chunks;
}

async function main() {
  if (!KLING_ACCESS_KEY || !KLING_SECRET_KEY) {
    console.error('ERROR: KLING_ACCESS_KEY and KLING_SECRET_KEY must be set in .env');
    process.exit(1);
  }

  const pending = ALL_CLIPS.filter(c => {
    const dest = path.join(CLIPS_ROOT, c.dir, c.file);
    if (existsSync(dest)) { console.log(`  [skip] ${c.name} already exists`); return false; }
    return true;
  });

  const total = pending.length;
  const batches = chunkOf3(pending);
  console.log(`\nRound 4 — ${total} clips × ${CREDITS_PER_CLIP} credits = ${total * CREDITS_PER_CLIP} credits`);
  console.log(`Estimated: ~${estimatedCredits} available → ~${estimatedCredits - total * CREDITS_PER_CLIP} after (floor: ${CREDIT_FLOOR})\n`);

  if (args['dry-run']) {
    for (const c of pending) console.log(`  [${c.dir}] ${c.name}: ${c.prompt.substring(0, 90)}...`);
    return;
  }

  let completed = 0;
  for (const [bi, batch] of batches.entries()) {
    const creditsNeeded = batch.length * CREDITS_PER_CLIP;
    if (estimatedCredits - creditsNeeded < CREDIT_FLOOR) {
      console.log(`\n⛔ Credit guard: ~${estimatedCredits} remaining — would drop below ${CREDIT_FLOOR}. Stopped after ${completed}/${total}.`);
      break;
    }

    console.log(`\n--- Batch ${bi + 1}/${batches.length}: ${batch.map(c => c.name).join(', ')} ---`);
    const tasks = [];
    for (const clip of batch) {
      process.stdout.write(`  [${clip.name}] submitting...`);
      const taskId = await submit(clip.prompt);
      console.log(` → ${taskId}`);
      tasks.push({ ...clip, taskId });
    }
    estimatedCredits -= batch.length * CREDITS_PER_CLIP;
    console.log(`  Credits remaining: ~${estimatedCredits}`);

    const urls = await pollBatch(tasks);
    for (const [i, clip] of tasks.entries()) {
      const url = urls[i];
      if (!url) { console.log(`  ✗ ${clip.name} — failed`); continue; }
      const destDir = path.join(CLIPS_ROOT, clip.dir);
      mkdirSync(destDir, { recursive: true });
      const destPath = path.join(destDir, clip.file);
      process.stdout.write(`  Downloading ${clip.file}...`);
      console.log(` ${await download(url, destPath)} ✓`);
      completed++;
    }
  }

  console.log(`\n=== Round 4 complete — ${completed}/${total} clips ===`);
  console.log(`=== Estimated credits remaining: ~${estimatedCredits} ===\n`);
}

main().catch(err => { console.error('Fatal:', err.message); process.exit(1); });
